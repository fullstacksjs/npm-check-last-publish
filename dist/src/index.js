...
const GREEN_AREA = (target) => target < 180;
...
for (const packageName of Object.keys(fakePackageJSON.dependencies)) {
    ...
    const { stdout: packagePublishDateListStdout } = await execPromise(`npm view ${packageName} time --json`);
    const packagePublishDateList = JSON.parse(packagePublishDateListStdout);
    const packagePublishDate = packagePublishDateList[packageVersion];
    const diffInTime = new Date().getTime() - new Date(packagePublishDate).getTime();
    const diffInDays = Math.round(diffInTime / (1000 * 3600 * 24));
    let area = "red";
    if (GREEN_AREA(diffInDays))
        area = "green";
    if (YELLOW_AREA(diffInDays))
        area = "yellow";
    if (RED_AREA(diffInDays))
        area = "red";
...
for (const dependencyInfo of dependenciesInformation) {
    ...
    const date = dependencieArea(formatDistance(dependencyInfo.date, new Date(), { addSuffix: true }));
